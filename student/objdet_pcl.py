...
def show_pcl(pcl):
    ...
    vis.create_window(window_name="My Open3D Window", width=800, height=600)
    
    
    def key_callback_function(vis):
        ...
        #return True # Return True if geometry update is needed
    vis.register_key_callback(262, key_callback_function) # Register callback for 'A' key
    # step 2 : create instance of open3d point-cloud class
    pcd = o3d.geometry.PointCloud()
    # step 3 : set points in pcd instance by converting the point-cloud into 3d vectors (using open3d function Vector3dVector)
    pcd.points = o3d.utility.Vector3dVector(np.array(pcl[:,0:3]))
...
def show_range_image(frame, lidar_name):
    ...
    img_intensity = img_range_intensity[:,ri_center-deg45:ri_center+deg45]

    cv2.imshow('intensity image', img_range_intensity)
    cv2.waitKey(0)"""
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_top = np.zeros_like((configs.bev_width, configs.bev_height))
    lidar_pcl_cpy[lidar_pcl_cpy[:,3]>1.0,3] = 1.0
    # step 2 : re-arrange elements in lidar_pcl_cpy by sorting first by x, then y, then -z (use numpy.lexsort)
    idx_int = np.lexsort((-lidar_pcl_cpy[:, 3], lidar_pcl_cpy[:, 1], lidar_pcl_cpy[:, 0]))
    lidar_pcl_top = lidar_pcl_cpy[idx_int]
    ## step 3 : extract all points with identical x and y such that only the top-most z-coordinate is kept (use numpy.unique)
    ##          also, store the number of points per x,y-cell in a variable named "counts" for use in the next task
    _, idx_int_unique = np.unique(lidar_pcl_top[:, 0:2], axis=0, return_index=True)

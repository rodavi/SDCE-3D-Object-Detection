    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            for obj in detections:
                ...
                _id, x, y,z, _h, w, l, yaw =  np.array(obj).reshape(1,-1).squeeze()
                pred_bbox = tools.compute_box_corners(x, y, w, l, yaw)
                
                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                cen_dist_x = label.box.center_x - x
                cen_dist_y = label.box.center_y - y
                cen_dist_z = label.box.center_z - z
                dis = (cen_dist_x**2+cen_dist_y**2+cen_dist_z**2)**0.5
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                pol1 = Polygon(gt_bbox)
                pol2 = Polygon(pred_bbox)
                
                intersection = pol1.intersection(pol2).area
                union = pol1.union(pol2).area

                iou = intersection/union
                ## step 6 : if IOU exceeds min_iou threshold, store [iou,dist_x, dist_y, dist_z] in matches_lab_det and increase the TP count
                if iou > min_iou:
                    matches_lab_det.append([iou, cen_dist_x, cen_dist_y, cen_dist_z])
...
def compute_performance_stats(det_performance_all):
    ...
    #[ious, center_devs, pos_negs] = det_performance_all
